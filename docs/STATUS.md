...
`simanalysis bisect start|status|next|record-verdict|restore|handoff`. The True
Engine now has conservative read-only STBL v5 and SimData metadata parsers wired
into package scanning, with compact parser/resource truth exposed through CLI
JSON, the desktop bridge, HTTP scan, and WebSocket scan responses. Visual
inventory, visual cleanup, richer Doctor verdicts, and symptom playbooks remain
