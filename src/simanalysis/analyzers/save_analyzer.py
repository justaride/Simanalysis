...
from simanalysis.scanners.mod_scanner import Mod, ModScanner
from simanalysis.scanners.save_scanner import ResourceKey, SaveData, SaveScanner
...
class SaveAnalyzer:
    ...
    def analyze_save(
        ...
        mods = self.mod_scanner.scan_directory(
            mods_path, recursive=True, progress_callback=mod_progress_handler
        )
        ...
        if save_data.total_resources > 0:
            result.coverage_percentage = len(matched_resources) / save_data.total_resources * 100
