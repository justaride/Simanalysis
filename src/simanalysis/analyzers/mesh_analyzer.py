...
from simanalysis.models import Mod, ModConflict, ConflictType, Severity, DBPFResource
        ...
        for mod in mods:
            try:
                ...
                for res in reader.get_resources_by_type(self.TYPE_CAS_PART):
                    self._scan_dependencies(mod, reader, res, self.SIG_GEOM, self.TYPE_GEOM, required_meshes)
                    
                # Scan Object Definitions
                for res in reader.get_resources_by_type(self.TYPE_OBJECT_DEF):
                    self._scan_dependencies(mod, reader, res, self.SIG_MODEL, self.TYPE_MODEL, required_meshes)
