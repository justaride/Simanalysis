...
async def get_config():
    ...
    return {
        "last_scan_path": config_service.last_scan_path
    }
    ...
    try:
        ...
        if os.name == 'posix' and os.path.exists(os.path.expanduser('~/.Trash')):
            ...
            if trash_path.exists():
                trash_path = Path.home() / '.Trash' / f"{file_path.stem}_{int(datetime.now().timestamp())}{file_path.suffix}"
            shutil.move(str(file_path), str(trash_path))
            return {"status": "ok", "message": f"File moved to trash: {trash_path}", "moved_to_trash": True}
        else:
