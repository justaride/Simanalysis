...
- Added a read-only SQLite inventory ledger and `inventory-scan` bridge command that record scans, file identity, DBPF package parse status, resource keys, snapshots, event-log entries, changed-since-last-scan counts, and JSON-serializable snapshot exports.
