...
- Added a read-only SQLite inventory ledger plus bridge/Tauri `inventory-scan` plumbing that records scans, file identity, DBPF package parse status, resource keys, snapshots, event-log entries, changed-since-last-scan counts, and JSON-serializable snapshot exports.
