...
- Added a Tauri-first Public v3 release smoke harness and checklist for
  ...
  builds.
- Added a Public v3 release security gate for CycloneDX SBOM generation,
  Bandit, pip-audit, npm production audits, Cargo lock metadata checks, and
  explicit pending signing/notarization status.
- Added release artifact verification to the security gate so built artifacts
  can emit `release-artifact-status.json` and strict mode refuses unsigned or
